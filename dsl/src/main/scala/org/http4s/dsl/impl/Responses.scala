...
object Responses {
  final class ContinueOps[F[_], G[_]](val status: Continue.type) extends AnyVal with EmptyResponseGenerator[F, G]
  // TODO support Upgrade header
  final class SwitchingProtocolsOps[F[_], G[_]](val status: SwitchingProtocols.type) extends AnyVal with EmptyResponseGenerator[F, G]
  final class EarlyHintsOps[F[_], G[_]](val status: EarlyHints.type) extends AnyVal with EmptyResponseGenerator[F, G]
  final class OkOps[F[_], G[_]](val status: Ok.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]

  final class CreatedOps[F[_], G[_]](val status: Created.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class AcceptedOps[F[_], G[_]](val status: Accepted.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  ...
  final class NoContentOps[F[_], G[_]](val status: NoContent.type) extends AnyVal with EmptyResponseGenerator[F, G]
  final class ResetContentOps[F[_], G[_]](val status: ResetContent.type) extends AnyVal with EmptyResponseGenerator[F, G] {
  ...
  final class PartialContentOps[F[_], G[_]](val status: PartialContent.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class MultiStatusOps[F[_], G[_]](val status: Status.MultiStatus.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class AlreadyReportedOps[F[_], G[_]](val status: AlreadyReported.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class IMUsedOps[F[_], G[_]](val status: IMUsed.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]

  final class MultipleChoicesOps[F[_], G[_]](val status: MultipleChoices.type, val liftG: G ~> F) extends LocationResponseGenerator[F, G]
  final class MovedPermanentlyOps[F[_], G[_]](val status: MovedPermanently.type, val liftG: G ~> F) extends LocationResponseGenerator[F, G]
  final class FoundOps[F[_], G[_]](val status: Found.type, val liftG: G ~> F) extends LocationResponseGenerator[F, G]
  final class SeeOtherOps[F[_], G[_]](val status: SeeOther.type, val liftG: G ~> F) extends LocationResponseGenerator[F, G]
  final class NotModifiedOps[F[_], G[_]](val status: NotModified.type) extends AnyVal with EmptyResponseGenerator[F, G]
  ...
  final class BadRequestOps[F[_], G[_]](val status: BadRequest.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class UnauthorizedOps[F[_], G[_]](val status: Unauthorized.type) extends AnyVal with WwwAuthenticateResponseGenerator[F, G]
  final class PaymentRequiredOps[F[_], G[_]](val status: PaymentRequired.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class ForbiddenOps[F[_], G[_]](val status: Forbidden.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class NotFoundOps[F[_], G[_]](val status: NotFound.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class MethodNotAllowedOps[F[_], G[_]](val status: MethodNotAllowed.type) extends AnyVal with AllowResponseGenerator[F, G]
  final class NotAcceptableOps[F[_], G[_]](val status: NotAcceptable.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class ProxyAuthenticationRequiredOps[F[_], G[_]](
      val status: ProxyAuthenticationRequired.type,
      val liftG:  G ~> F,
  ) extends EntityResponseGenerator[F, G]
  // TODO send Connection: close?
  final class RequestTimeoutOps[F[_], G[_]](val status: RequestTimeout.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class ConflictOps[F[_], G[_]](val status: Conflict.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class GoneOps[F[_], G[_]](val status: Gone.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class LengthRequiredOps[F[_], G[_]](val status: LengthRequired.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class PreconditionFailedOps[F[_], G[_]](
      val status: PreconditionFailed.type,
      val liftG:  G ~> F,
  ) extends EntityResponseGenerator[F, G]
  final class PayloadTooLargeOps[F[_], G[_]](val status: PayloadTooLarge.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class UriTooLongOps[F[_], G[_]](val status: UriTooLong.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  ...
  final class LockedOps[F[_], G[_]](val status: Locked.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class FailedDependencyOps[F[_], G[_]](val status: FailedDependency.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class TooEarlyOps[F[_], G[_]](val status: TooEarly.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  // TODO Mandatory upgrade field
  final class UpgradeRequiredOps[F[_], G[_]](val status: UpgradeRequired.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class PreconditionRequiredOps[F[_], G[_]](
      val status: PreconditionRequired.type,
      val liftG:  G ~> F,
  ) extends EntityResponseGenerator[F, G]
  final class TooManyRequestsOps[F[_], G[_]](val status: TooManyRequests.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  ...
  final class NotImplementedOps[F[_], G[_]](val status: NotImplemented.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class BadGatewayOps[F[_], G[_]](val status: BadGateway.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class ServiceUnavailableOps[F[_], G[_]](
      val status: ServiceUnavailable.type,
      val liftG:  G ~> F,
  ) extends EntityResponseGenerator[F, G]
  final class GatewayTimeoutOps[F[_], G[_]](val status: GatewayTimeout.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  ...
  final class LoopDetectedOps[F[_], G[_]](val status: LoopDetected.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
  final class NotExtendedOps[F[_], G[_]](val status: NotExtended.type, val liftG: G ~> F) extends EntityResponseGenerator[F, G]
