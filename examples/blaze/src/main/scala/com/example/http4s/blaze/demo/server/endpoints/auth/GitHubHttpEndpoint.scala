...
class GitHubHttpEndpoint[F[_]](gitHubService: GitHubService[F])(implicit F: Sync[F]) extends Http4sDsl[F] {
  ...
  val service: HttpRoutes[F] = HttpRoutes.of {
    ...
    case GET -> Root / ApiVersion / "login" / "github" :? CodeQuery(code) :? StateQuery(state) =>
      ...
      } yield o.withEntity(code).putHeaders("Content-Type" -> "application/json")
  }
}
